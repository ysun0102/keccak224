 ...
 * p is not null
 */
void zdd_rename_write_to_files(FILE *f, DdManager *zdd, DdNode *p) //r
{
    CHECK_NN(p)
    ...
    if (p == zero)
        ...
        fprintf(f, "x%d",  p->index < PARAM_ST ? p->index : p->index - PARAM_ST);
        if (t != one) {
            fprintf(f, "*");
            if (cuddE(t) == zero) {
                zdd_rename_write_to_files(f, zdd, t);
            } else {
                fprintf(f, "(");
                zdd_rename_write_to_files(f, zdd, t);
                fprintf(f, ")");
        ...
        if (e == zero) {
            ...
            fprintf(f, "+");
            zdd_rename_write_to_files(f, zdd, e);
        }
    }
}
...
void zdd_write_to_files(FILE *f, DdManager *zdd, DdNode *p) //r
{
    CHECK_NN(p)
    ...
    if (p == zero)
        ...
        fprintf(f, "x%d",  p->index);
        if (t != one) {
            fprintf(f, "*");
            if (cuddE(t) == zero) {
                zdd_write_to_files(f, zdd, t);
            } else {
                fprintf(f, "(");
                zdd_write_to_files(f, zdd, t);
                fprintf(f, ")");
...
void zdd_nonlinear_poly_write_to_files( FILE *f, zddpoly_t *zddp){
    ...
    for (idrc_t i = 0; i < zddp->size_of_polys; i++) {
        zdd_rename_write_to_files(f, zddp->zdd, zddp->polys[i]);
...
void zdd_linear_poly_write_to_files( FILE *f, zddpoly_t *zddp){
    ...
    for (var_subscript_t i = 0; i < zddp->rank_of_linear; i++) {
        zdd_write_to_files(f, zddp->zdd, zddp->linear[i]);
